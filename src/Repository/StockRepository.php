...
{
    return $this->createQueryBuilder('s')
        ->select('MIN(s.id) as stock_id, MAX(s.image) as image, MAX(s.prix_produit) as prix_produit, SUM(s.quantity) as quantity, p.nom_produit, p.id as produit_id, p.description')
